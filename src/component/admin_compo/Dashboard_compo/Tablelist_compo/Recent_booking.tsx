   ...
   <div className="bg-[#0b111e] text-white p-8 rounded-2xl w-full border border-slate-800/60 shadow-2xl font-sans">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="text-[10px] uppercase tracking-widest text-slate-500 border-b border-slate-800/10">
              <th className="pb-5 px-5 font-semibold">Customer Name</th>
              <th className="pb-5 px-5 font-semibold">Outlet</th>
          ...
          <tbody className="divide-y divide-slate-800/20">
            {bookings.map((booking) => (
              <tr key={booking.id} className="group hover:bg-[#11192a]/60 transition-colors">
                ...
                {/* Outlet */}
                <td className="py-5 px-8 text-sm text-slate-400 tracking-tight">
                  {booking.outlet}
                </td>

                {/* Date */}
                <td className="py-5 px-4 text-sm text-slate-400 whitespace-nowrap tracking-tight">
                  {booking.date}
                </td>
